...
from .custom_recognition import get_all_cropped_image_and_max_width, batch_get_text
...
class CustomReader(object):
    ...
    def recognize(self, img_cv_grey_list, batch_horizontal_list=None, batch_free_list=None, \
                  ...
                  reformat=True):
        result_list = []
        batch_image_list, max_width = get_all_cropped_image_and_max_width(batch_horizontal_list,
                                                                          batch_free_list,
                                                                          img_cv_grey_list,
                                                                          model_height=64)
        # print("batch_image_list", batch_image_list)
        ...
        if self.model_lang in ['chinese_tra', 'chinese_sim', 'japanese', 'korean']:
            decoder = 'greedy'
        result, cut_point_list = batch_get_text(self.character, imgH, int(max_width), self.recognizer, self.converter, batch_image_list, \
    ...
    def back_to_original_point(self, image_list, resized_image_list, batch_horizontal_list, batch_free_list):
        for i in range(0, len(image_list)):
            ...
            for j in range(0, len(free_list)):
                ...
                x1, x2, x3, x4 = int(x1 * width_ratio), int(x2 * width_ratio), int(x3 * width_ratio), int(
                    x4 * width_ratio)
                y1, y2, y3, y4 = int(y1 * height_ratio), int(y2 * height_ratio), int(y3 * height_ratio), int(
                    y4 * height_ratio)
                free_list[j] = [[x1, y1], [x2, y2], [x3, y3], [x4, y4]]

    def convert_result2cut_result(self, result, cut_point_list):
        cut_result = []
        previous_cut_point = 0
        temp_list = []
        count = 0
        for cut_point in cut_point_list:
            if cut_point != previous_cut_point:
                previous_cut_point = cut_point
                cut_result.append(temp_list)
                temp_list = []
            
            area, text, _, _ = result[count]
            temp_list.append([area, text])
            count = count + 1
        return cut_result
    ...
    def readtext(self, image_list, decoder='greedy', beamWidth=5, batch_size=1, \
        ...
        print("aaaaaaaa")
        result, cut_point_list = self.recognize(img_cv_grey_list, batch_horizontal_list, batch_free_list, \
        ...
        cut_result_list = self.convert_result2cut_result(result, cut_point_list)
        for cut_result in cut_result_list:
            print(cut_result)
